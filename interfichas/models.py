...
class Disciplina(models.Model):
    ...
    tipo_marcador     = models.CharField(
        ...
        default='goles',
        help_text='Define cómo se registra el resultado de cada partido'
    )
...
class PartidoInterfichas(models.Model):
    ...
    equipo_visitante = models.ForeignKey(
        EquipoInterfichas, on_delete=models.CASCADE, related_name='partidos_visitante'
    )
    ...
    @property
    def tipo_marcador(self):
        """Acceso rápido al tipo de marcador de la disciplina."""
        try:
            return self.torneo.disciplina.tipo_marcador
        except Exception:
            return 'goles'

    @property
    def sets_str(self):
        """Devuelve string bonito de parciales: '25-20, 23-25, 15-10'"""
        if not self.sets_local or not self.sets_visitante:
            return ''
        pares = zip(self.sets_local, self.sets_visitante)
        return ', '.join(f"{a}-{b}" for a, b in pares)
